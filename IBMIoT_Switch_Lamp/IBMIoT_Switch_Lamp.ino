...
void handleUserCommand(JsonObject* root) {
    ...
    if(d.containsKey("color")){
      ...
      blue = atoi(d["color"]["b"]);
      sprintf(r_buffer,"r:%3d",red);
      sprintf(g_buffer,"g:%3d",green);
      sprintf(b_buffer,"b:%3d",blue);
      
      display.print(r_buffer,2,2);
      display.print(g_buffer,2,7);
      display.print(b_buffer,3,2);
...
void setup() {
    Serial.begin(115200);
    pixels = Adafruit_NeoPixel(ledNum,ledPin,NEO_RGB+NEO_KHZ800);
    pixels.begin();
    pixels.show();
    initCheck();
    ...
    display.begin();
    display.print("IoT Color Picker");
}
